...
// Variables for loading screen - Tuathal 12/04
PImage loadingScreenImage;
PFont loadingScreenFont;
...
void setup()
{
  ...
  y = 20;
  size(900, 600);
  loadingScreenImage = loadImage("realPlane3.png");
  loadingScreenFont = loadFont("Arial-ItalicMT-56.vlw");
  textFont(loadingScreenFont);
  loadingScreenOn = true;
  loadingData = true;
}

// Called while on loading screen - Tuathal 12/04
void setupAll(){
...
{
  if(loadingScreenOn)
  {                   // draws loading screen - Tuathal 12/04
    background(#e5eaf5);
    image(loadingScreenImage, 0, 100);
    fill(0);
    text("Not Ready Yet...", 400, 500);
    loadingScreenOn = false;
  }
  else if(loadingData)
  {                  // loads data while on loading screen - Tuathal 12/04
    setupAll();
    loadingData = false;
  }
  else{                                 // proceeds to welcome screen 12/04 - Tuathal 12/04
    background(255);
